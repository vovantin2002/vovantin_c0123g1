package case_study_module2.util.read_and_write;
import case_study_module2.model.person.Customer;
import java.io.*;
import java.util.ArrayList;
import java.util.List;

public class ReadAndWriteCustomer {
    private static final String CUSTOMER_LIST_PATH = "src\\case_study_module2\\data\\customer.csv";

    public static void writeEmployeeToFile(Customer customer) {
        File file = new File(CUSTOMER_LIST_PATH);
        try {
            FileWriter fileWriter = new FileWriter(file, true);
            fileWriter.write(customer.getInfoToCsv());
            fileWriter.write("\n");
            fileWriter.close();
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    public static List<Customer> readStudentFromFile() {
        ArrayList<Customer> customerlist = new ArrayList();
        File file = new File(CUSTOMER_LIST_PATH);
        try {
            FileReader fileReader = new FileReader(file);
            BufferedReader bufferedReader = new BufferedReader(fileReader);
            String str = "";
            while ((str = bufferedReader.readLine()) != null && !(str).equals("")) {
                String[] strings = str.split(",");
                Customer customer1 = new Customer(strings[0], strings[1], strings[2],
                        strings[3], strings[4], strings[5], strings[6], strings[7], strings[8]);
                customerlist.add(customer1);
            }bufferedReader.close();
            fileReader.close();
        } catch (IOException e) {
            throw new RuntimeException();
        }
        return customerlist;
    }
}
